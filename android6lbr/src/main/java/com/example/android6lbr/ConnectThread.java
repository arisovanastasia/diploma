...
public class ConnectThread extends Thread {
    ...
    */
    ...
    public ConnectThread(Autoconnect connManager, BluetoothDevice device) {
        ...
        String[] bdaOwnDevice = BluetoothAdapter.getDefaultAdapter().getAddress().split(":"); // Can't get address this way
    ...
    private void connectThroughL2capCoc() {
        ...
        Log.d("###", "size of open connections: " + mConnManager.bluetoothConnections.size());
    ...
    private void analyzeMessageContent(byte[] inputMessage, IPHC compressedHeader, int bytePos) {
        ...
        if (compressedHeader.nextHeader == (byte)0x3A) {
            ...
            if (type == (byte) 0x85) {
                ...
                Log.d("###BDA", BluetoothAdapter.getDefaultAdapter().getAddress()); // Can't get address this way
    ...
    private boolean routeMessage(byte[] message, byte[] targetAddress) {
        ...
        for (ConnectThread thread : mConnManager.bluetoothConnections) {
    ...
    private void answerUdpEchoRequest(byte[] inputMessage, int bytePosContent, IPHC compressedHeader) {
        ...
        // TODO: remove this
        // POSTER SESSION ONLY
        // Here we can check the UDP datagram content and call something upper-layer
        if(true){
            mConnManager._liveData.postValue(bytesToHex(content));
        }
        // POSTER SESSION ONLY
        // TODO: remove this
